    ...
    def get_opt_state_spec_psgd():
        ...
        def shard_psgd_precond(Qs: list):
            ...
            for precond in Qs:
                ...
                if len(shape) < 2 or (len(shape) == 2 and shape[-2] != shape[-1]):
        ...
        for k, v in opt_state_shapes[psgd_idx].items():
            if k == "count":
                ...
                psgd_specs[k] = params_spec_for_opt
            elif k == "Qs_preconditioners":
                # preconditioners (kept in lists)
                precond_specs = jax.tree.map(shard_psgd_precond, v, is_leaf=lambda x: isinstance(x, list))
